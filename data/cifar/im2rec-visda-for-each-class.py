...
def list_image(root, recursive, exts, label_value):
    i = 0
    if recursive:
        cat = {}
        for path, dirs, files in os.walk(root, followlinks=True):
            ...
            for fname in files:
                ...
                if os.path.isfile(fpath) and (suffix in exts):
                    if path not in cat:
                        cat[path] = len(cat)
                    yield (i, os.path.relpath(fpath, root), label_value)
                    i += 1
...
def make_list(args):
    image_list = list_image(args.root, args.recursive, args.exts, args.label_value)
...
def parse_args():
    ...
    cgroup.add_argument('--label_value', type=int, default=0,
                        help='label of the images.')
